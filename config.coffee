exports.config =
  ...
  paths:
    ignored: (path) ->
      path.match(/^vendor(\/|\\)styles(\/|\\)bootstrap/) ||
      (path is 'app/config_development.coffee' && process.env.PRODUCTION) ||
      (path is 'app/config_production.coffee' && !process.env.PRODUCTION)
